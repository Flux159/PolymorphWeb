...
public class OVRPlayerController : OVRComponent
{
		...
		new public virtual void Update ()
		{
				...
				if (!NoGravity) {
						...
						//if(GamePad.CopyGetAxis(GamePad.Axis.LeftYAxis) != 0)
						Controller.Move (moveDirection);
				}
		...
		public virtual void UpdateMovement ()
		{
				...
				if (!Controller.isGrounded && !NoGravity)	
						MoveScale = 0.0f;
				...
				XRotation -= rotateInfluence * 0.5f;
				...
				if (AllowMouseRotation == false)
						deltaRotation = Input.GetAxis ("Mouse X") * rotateInfluence * 1.00f;

				float upDownRotation = 0.0f;
				if (AllowMouseRotation == false)
						upDownRotation = Input.GetAxis ("Mouse Y") * rotateInfluence * 1.00f;
				XRotation -= upDownRotation;
